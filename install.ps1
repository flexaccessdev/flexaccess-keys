...
# flexaccess-keys installer for Windows
...
function Get-BinaryName {
    ...
    return "flexaccess-keys-windows-amd64.exe"
}
...
function Install-Binary {
    ...
    $url = "$BaseUrl/$BinaryName"
    $tempDir = Join-Path $env:TEMP "flexaccess-keys-install-$(Get-Random)"
    $tempBinary = Join-Path $tempDir $BinaryName
    $installDir = Join-Path $env:LOCALAPPDATA "Programs\flexaccess-keys"
    $finalPath = Join-Path $installDir "flexaccess-keys.exe"
...
Download and install flexaccess-keys binary
...
function Start-Installation {
    ...
    if ($DownloadOnly) {
        Print-Info "flexaccess-keys downloader"
    }
    else {
        Print-Info "flexaccess-keys installer"
    }
    ...
    if ($DownloadOnly) {
        ...
        Print-Info "You can now run 'flexaccess-keys' from your terminal."
    }
}
...
function Main {
    ...
    if ($Help -or $args -contains "--help" -or $args -contains "-h" -or $args -contains "-?" -or $args -contains "/?" -or $args -contains "/h" -or
    ...
    if ($DownloadOnly) {
        Print-Info "Starting flexaccess-keys download..."
    }
    else {
        Print-Info "Starting flexaccess-keys installation..."
    }
